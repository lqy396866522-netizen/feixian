...
export class TabBarView {
    ...
    build() {
        ...
        this.tabIndicator = this.f.mk('tabInd', bar, 76, 5, 0, -62);
        ...
        TAB_ORDER.forEach((id, i) => {
            ...
            const b = this.f.mk('tab_' + id, bar, 82, 110, x, 4);
            this.tabDots[id] = b;
            const icon = this.f.mk('ic', b, 68, 68, 0, 12);
            this.tabIcons[id] = icon;
            this.f.loadSprite(icon, meta.icon, 68, 68, false, () => {
                this.f.label(b, meta.label.charAt(0), 20, C.tabGlyph, 0, 6, 60, 28, true);
            });
            this.tabLabels[id] = this.f.label(b, meta.label, 15, meta.enabled ? C.goldLt : C.disabled, 0, -36, 82, 22, true);
    ...
    refresh(current: TabId) {
        ...
        this.tabIndicator.setPosition(x, -62, 0);
        ...
        TAB_ORDER.forEach((id) => {
            ...
            const path = id === 'play' && selected
                ...
                : meta.icon;
            this.f.loadSprite(icon, path, 68, 68, false);
            if (lb) {
