  ...
  return {
    ...
    execute: function () {
      ...
      var GameModel = exports('GameModel', /*#__PURE__*/function () {
        ...
        _proto.playSfx = function playSfx(_name) {
          if (!this.sfxEnabled) return;
          // MVP: no clip assets yet; toggle + persist is the deliverable
        };

        _proto.getActiveBeast = function getActiveBeast() {
          var id = this.save.activeBeastId;
          if (!id || !this.save.beasts) return null;
          return this.save.beasts.find(function (b) {
            return b.id === id;
          }) || null;
        };
        _proto.deployBeast = function deployBeast(id) {
          var b = (this.save.beasts || []).find(function (x) {
            return x.id === id;
          });
          if (!b) return false;
  ...
  return {
    ...
    execute: function () {
      ...
      var C = {
        ...
        tabOn: new Color(46, 204, 113, 255),
        // #2ECC71
        tabGlyph: new Color(232, 244, 255, 255),
        // #E8F4FF
      ...
      var MainGame = exports('MainGame', (_dec = ccclass('MainGame'), _dec(_class = /*#__PURE__*/function (_Component) {
        ...
        function MainGame() {
          ...
          _this.settingsRoot = void 0;
          _this.beastsRoot = void 0;
          _this.lblBeastName = void 0;
          _this.lblBeastPower = void 0;
          _this.beastSpNode = void 0;
          _this.sfxIconNode = void 0;
          _this.lblSfx = void 0;
        ...
        _proto.buildBeasts = function buildBeasts() {
          ...
          // actions
          var deploy = this.mk('deploy', r, 200, 56, -140, -200);
          this.fill(deploy, C.primary, 14);
          this.label(deploy, '上阵', 24, C.white, 0, 0, 180, 40, true);
          this.click(deploy, function () {
            _this6.model.deployBeast('beast_fox');
            _this6.showToast('已上阵 青蔓灵狐');
            _this6.refreshBeastPage();
            _this6.model.playSfx('click');
          });
          var swap = this.mk('swap', r, 200, 56, 140, -200);
          this.fill(swap, C.navy2, 14);
          this.label(swap, '更换', 24, C.white, 0, 0, 180, 40, true);
          this.click(swap, function () {
            return _this6.showToast('MVP 仅 1 只异兽');
          });

          // locked
          var grow = this.mk('grow', r, 200, 56, -140, -280);
          this.fill(grow, C.disabledBg, 14);
          this.label(grow, '养成', 22, C.disabled, 0, 0, 180, 40, true);
          this.click(grow, function () {
            return _this6.showToast('养成未开放');
          });
          var skill = this.mk('skill', r, 200, 56, 140, -280);
          this.fill(skill, C.disabledBg, 14);
          this.label(skill, '技能', 22, C.disabled, 0, 0, 180, 40, true);
          this.click(skill, function () {
            return _this6.showToast('技能未开放');
          });
        };
        _proto.refreshBeastPage = function refreshBeastPage() {
          var b = this.model.getActiveBeast();
        ...
        _proto.buildSettings = function buildSettings() {
          ...
          var btn = this.mk('sfxBtn', row, 120, 44, 280, 0);
          this.fill(btn, C.primary, 12);
          this.label(btn, '切换', 20, C.white, 0, 0, 100, 36, true);
          this.click(btn, function () {
            return _this7.onToggleSfx();
          });
          this.click(row, function () {
            return _this7.onToggleSfx();
          });
        };
        _proto.onToggleSfx = function onToggleSfx() {
          var on = this.model.toggleSfx();
        ...
        _proto.buildTabBar = function buildTabBar() {
          ...
          var barH = 120;
          var bar = this.mk('TabBar', this.root, DESIGN_W, barH, 0, -580);
          ...
          var n = tabs.length;
          var gap = DESIGN_W / n;
          var startX = -DESIGN_W / 2 + gap / 2;
          tabs.forEach(function (t, i) {
            var x = startX + i * gap;
            var b = _this8.mk('tab_' + t.id, bar, 88, 88, x, 10);
            _this8.tabDots[t.id] = b;
            _this8.paintTabDot(b, t.id === _this8.tab, t.enabled);
            var gl = _this8.label(b, t.glyph, 26, t.enabled ? C.tabGlyph : C.disabled, 0, 6, 70, 36, true);
            _this8.tabLabels[t.id] = _this8.label(b, t.name, 14, t.enabled ? C.goldLt : C.disabled, 0, -28, 80, 22);
            _this8.click(b, function () {
              if (!t.enabled) {
                _this8.showToast(t.name + " \u672A\u5F00\u653E");
                return;
              }
              _this8.setTab(t.id);
            });
        ...
        _proto.paintTabDot = function paintTabDot(n, selected, enabled) {
          var ui = n.getComponent(UITransform) || n.addComponent(UITransform);
          var d = !enabled ? 84 : selected ? 100 : 88; // selected +4px vs prior 96
        ...
        _proto.loadSprite = function loadSprite(node, path, w, h, footAnchor) {
          ...
          var tryApply = function tryApply(sf, via) {
            if (!sf) return false;
            _this10.applySpriteFrame(node, sf, w, h, footAnchor);
        ...
        _proto.buildChestBtn = function buildChestBtn() {
          ...
          this.click(this.chestBtn, function () {
            if (_this11.model.tryOpenChest()) {
              _this11.showLootPanel(true);
              _this11.refreshAll();
            }
          });
        };
        ...
        _proto.refreshTabs = function refreshTabs() {
          ...
          Object.keys(this.tabDots).forEach(function (id) {
            var enabled = id === 'play' || id === 'equip' || id === 'spells' || id === 'guild' || id === 'beasts';
            var n = _this12.tabDots[id];
            if (!n) return;
            _this12.paintTabDot(n, id === _this12.tab, enabled);
            var lb = _this12.tabLabels[id];
            if (lb) {
              lb.color = enabled ? C.goldLt : C.disabled;
            }
            // glyph is first Label child named lbl near top — keep white/tabGlyph via children
            for (var _iterator2 = _createForOfIteratorHelperLoose(n.children), _step2; !(_step2 = _iterator2()).done;) {
              var ch = _step2.value;
              var lab = ch.getComponent(Label);
              if (!lab) continue;
              if (lab.fontSize >= 22) {
                lab.color = enabled ? id === _this12.tab ? C.white : C.tabGlyph : C.disabled;
              }
            }
          });
        };
        ...
        _proto.refreshAll = function refreshAll() {
          ...
          Object.keys(SLOT_LABELS).forEach(function (slot) {
            var lb = _this13.slotLabels[slot];
            if (!lb) return;
      ...
      var SaveSystem = exports('SaveSystem', /*#__PURE__*/function () {
        ...
        SaveSystem.load = function load() {
          try {
            ...
            if (merged.activeBeastId === undefined) merged.activeBeastId = 'beast_fox';
            if (!Array.isArray(merged.beasts) || !merged.beasts.length) {
              merged.beasts = [{
                id: 'beast_fox',
                name: '青蔓灵狐',
                power: 1260
